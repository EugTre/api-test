...
EXC_MSG__INVALID_POINTER_TYPE = \
    'Invalid pointer "{pointer_str}". '\
    'Pointer must be valid JSON pointer (e.g. "/a/b/c").'
EXC_MSG__UPDATE_ROOT_ERROR = \
    'Direct root modifications is not allowed! ' \
    'Specified pointer is required to add/update keys!'
EXC_MSG__LIST_INDEX_ERROR = \
    'Update failed due to invalid list index "{key}" at '\
    'node "/{path}" of pointer "{pointer}". {hint}'
EXC_MSG__INVALID_STORAGE = \
    'Not possible to add new key/element to path node "/{path}" '\
    'at pointer "{pointer}", target node is not a dict or list.'
EXC_MSG_HINT__FORMAT = \
    'Index must be an integer in list\'s range (to update '\
    'element at specific index) or "-" (to append new element).'


class JsonWrapper:
    """Class to wrap JSON data with get/update/delete methods that use
    JSON Pointers.

    Internally creates flattened representation of content that provides
    fast access to the nodes and keeps this representation up-to-date when
    .update()/.delete() methods are used.

    Note: After direct modifications made to the content one should use
    .refresh() method to restore integrity of internal reference structure!
    """
    __slots__ = ["node_map", "_content"]

    def __init__(self, content: dict | list):
        self._content: dict | list = content
        self.node_map: dict[Pointer, tuple[dict | list, str]] = None
        self.refresh()
    ...
    def refresh(self):
        ...
        """
        self.node_map = dict(self.__scan_storage(self._content))

    def __scan_storage(self, storage: list | dict,
                       pointer_path: tuple = tuple()) -> tuple[str, tuple]:
        """Deeply scans a storage and yield JSON pointer-like key and
        storage/key pairs to represent flat document structure.
        ...
        Yields:
            tuple[str, tuple]: JSON Pointer like key and tuple of storage and
            key pointer is refer to
        """
        is_list_storage = isinstance(storage, list)

        iter_source = enumerate(storage) \
            if is_list_storage else \
            storage.items()
    ...
    def get(self, pointer: str | Pointer) -> Any:
        ...
        Raises:
            ...
            KeyError: when key is missing from pre-calculated document
            structure map.
    ...
    def delete(self, pointer: str | Pointer) -> bool:
        ...
        if pointer not in self.node_map:
            return False

        storage, key = self.__unsafe_get_storage(pointer)
        del storage[key]
        if isinstance(storage, list):
            # In case of deleting list element - recalculate entire storage,
            # because elements may shift, invalidating entire subsection of
            # nodes map
            ...
            # For dict it should be ok to remove map for specific key and it's
            # childs if key contained container.
            del self.node_map[pointer]
    ...
    def update(self, pointer: str | Pointer, value: Any) -> bool:
        ...
        if pointer not in self.node_map:
            ...
            if isinstance(storage, list):
                ...
                self.node_map[pointer] = (storage, new_index)
            elif isinstance(storage, dict):
                storage[new_key] = value
                self.node_map[pointer] = (storage, new_key)
            else:
        ...
        # Update case
        # - delete stale nodes for given pointer (e.g. pointers to list items)
        # - change value
        # - if new value is container - calculate nested nodes and add to map
        storage, key = self.__unsafe_get_storage(pointer)

        if isinstance(storage[key], (list | dict)):
            self.__delete_stale_nodes(pointer)

        storage[key] = value
        self.node_map[pointer] = (storage, key)
    ...
    def __unsafe_get_storage(self, pointer: Pointer
                             ) -> tuple[list | dict, str]:
    ...
    def __add_new_nodes(self,  storage_pointer: Pointer):
        ...
        new_nodes = self.__scan_storage(
            self.__unsafe_get(storage_pointer),
            storage_pointer.path
        )
        self.node_map.update(new_nodes)

    def __delete_stale_nodes(self, storage_pointer: Pointer):
        """Delete existing child nodes of given pointer"""
        stale_nodes = [node
                       for node in self.node_map
                       if node.is_child_of(storage_pointer)]
    ...
    def __contains__(self, pointer: str | Pointer) -> bool:
        ...
        """
        pointer = self.__convert_to_pointer_obj(pointer)
        return pointer in self.node_map

    def __iter__(self):
        for ptr in self.node_map.keys():
